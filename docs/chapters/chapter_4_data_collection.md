...
**Malicious URLs:**
- **OpenPhish** (`openphish.com/feed.txt`): Live feed of active phishing URLs. Updated continuously. Free tier provides ~500 URLs; commercial feed provides full volume with brand/ASN metadata.
- **URLhaus** (`urlhaus.abuse.ch`): Malware distribution URLs. Specialized feeds by country, ASN, and TLD available. Rate limit: fetch no more than once per 10 minutes.

**Benign URLs:**
- **Tranco** (`tranco-list.eu`): Research-grade ranked list of popular domains, replacing the deprecated Alexa Top 1M. Use top-ranked domains as benign samples — high-rank domains are overwhelmingly legitimate.

## Dataset Construction

1. Fetch malicious URLs from OpenPhish and URLhaus; label as `1`
2. Sample domains from Tranco top-N; construct bare URLs (`https://domain.tld`); label as `0`
3. Target a balanced or mildly imbalanced ratio (e.g. 1:2 malicious:benign) — severe imbalance degrades recall on the minority class

## Deduplication and Hygiene

- Deduplicate on normalized URL (lowercased scheme+host+path, stripped trailing slash)
- Remove URLs with private/reserved IP ranges as host
- Remove URLs where the domain is in both malicious and benign sets (ambiguous label)
- Strip query parameters for dedup purposes only; retain full URL for feature extraction

## Temporal Considerations

Phishing URLs are short-lived. A URL collected today may resolve to a parked page tomorrow. For training, use a snapshot with a known collection date. For live detection, feed freshness matters — stale feeds miss current campaigns.

## Key Takeaway

Dataset quality determines model ceiling. Garbage labels produce garbage classifiers. Invest in deduplication, source freshness, and balanced sampling before tuning any model hyperparameters.
