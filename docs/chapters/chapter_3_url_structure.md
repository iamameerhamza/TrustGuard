...
A URL has the form: `scheme://subdomain.domain.tld/path?query#fragment`. Each component carries signal:

- **Scheme**: `http` vs `https` — absence of HTTPS is a weak negative signal; presence is not a positive one
- **Netloc** (`subdomain.domain.tld`): the registrable domain is `domain.tld`; everything left of it is subdomain
- **Path and query**: can contain obfuscated tokens, encoded characters, or brand names used to deceive

Use Python's `urllib.parse.urlparse()` and `tldextract` to split these reliably. Never split on `.` or `/` manually — edge cases (IPv6, IDN, ports) will break naive parsers.

## Lexical Features

These are computed from the raw URL string without any network request:

| Feature | Phishing signal |
|---|---|
| URL length | Longer URLs correlate with obfuscation |
| Dot count | Many dots suggest deep subdomains |
| Hyphen count in domain | Hyphens are rare in legitimate brand domains |
| Digit count in domain | Digits suggest auto-generated or typosquat domains |
| `@` symbol presence | Forces browser to treat left side as credentials |
| Double slash `//` in path | Redirect obfuscation |
| Suspicious keywords | `login`, `secure`, `verify`, `account`, `update`, `confirm` |
| TLD type | Free TLDs (`.tk`, `.ml`, `.ga`, `.cf`) are heavily abused |

## Obfuscation Techniques

Attackers manipulate URL appearance to bypass filters and deceive users:

- **IP address as host**: `http://192.168.1.1/paypal` — avoids domain registration
- **Hex/octal encoding**: `http://0xC0A80101/` — same IP, different representation
- **URL encoding**: `%70%61%79%70%61%6C` decodes to `paypal`
- **Punycode/IDN**: `xn--pypal-4ve.com` renders as `pаypal.com` with Cyrillic `а`
- **Subdomain abuse**: `paypal.com.attacker.net` — the real domain is `attacker.net`

## Key Takeaway

Lexical features are fast, require no network access, and catch a large fraction of phishing URLs. They form the first layer of TrustGuard's signal extraction before any external lookup is made.
