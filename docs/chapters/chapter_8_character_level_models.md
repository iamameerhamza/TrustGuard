...
Manual feature extraction has a ceiling; attackers constantly invent new obfuscation techniques to bypass hardcoded rules. To counter this, TrustGuard explores Character-Level Convolutional Neural Networks (Char-CNNs) as an advanced secondary detection layer.

Char-CNNs read URLs like images, utilizing sliding convolutional filters to learn spatial character relationships. This allows them to instantly recognize typosquatting, homograph attacks, and algorithmically generated domains (DGAs) without relying on predefined English dictionaries or rules.

While incredibly powerful and language-independent, Char-CNNs operate as a complete "Black Box" with zero explainability and high computational costs. Therefore, they are utilized as a heavy-duty backup to catch the most highly obfuscated anomalies that our primary Random Forest model might miss.
