...
Machine learning requires mathematical inputs. We parse URLs into three feature categories: Lexical (structural elements like length, entropy, and keywords), Trust (infrastructure metadata like domain age and SSL presence), and Reputation (threat feed hits). 

Features serve two distinct purposes in TrustGuard. Internal scoring features (like Shannon entropy or digit ratios) are critical for ML models to detect subtle algorithmic anomalies, even if they are opaque to humans. User-friendly features (like domain age or keyword presence) are routed to the explainability engine to generate readable warnings. 

This duality—complex math for detection paired with simple heuristics for explanation—mirrors industry-leading threat intelligence platforms and forms the foundation of our transparent detection architecture.
